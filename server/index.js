...
async function searchDuckDuckGo(query){try{const res=await axios.get(`https://api.duckduckgo.com/?q=${encodeURIComponent(query)}&format=json&no_html=1&skip_disambig=1`,{timeout:8000});const data=res.data;const results=[];if(data.AbstractURL&&data.AbstractTitle)results.push({title:data.AbstractTitle,url:data.AbstractURL,snippet:data.AbstractText});if(data.RelatedTopics){for(const t of data.RelatedTopics.slice(0,10)){if(t.FirstURL&&t.Text)results.push({title:t.Text.split(' - ')[0]||t.Text,url:t.FirstURL,snippet:t.Text});}}return results;}catch(err){console.error('DDG:',err.message);return[];}}
...
async function searchAllSourcesParallel(niche, country, searchTerms, jobFilter) {
  ...
  for (const ddgResult of [ddg0,ddg1,ddg2]) if (ddgResult.status === 'fulfilled') for (const r of ddgResult.value.slice(0,6)) {if(!r.url||r.url.includes('duckduckgo')||r.url.includes('wikipedia'))continue;leads.push({id:makeId('ddg'),companyName:extractCompanyName(r.title),website:r.url,ownerName:'',contactTitle:'',email:'',instagram:'',whatsapp:'',phone:'',linkedinUrl:buildLinkedInSearchUrl(extractCompanyName(r.title),jobFilter,country),country,source:'Web Search',dateFound:today(),score:0,painSignal:false,snippet:r.snippet||''}); }
  ...
  if (webSearch.status === 'fulfilled') for (const x of webSearch.value.slice(0,5)) if(!x.url.includes('duckduckgo')&&!x.url.includes('wikipedia')) leads.push({id:makeId('web'),companyName:extractCompanyName(x.title),website:x.url,ownerName:'',contactTitle:'',email:'',instagram:'',whatsapp:'',phone:'',linkedinUrl:buildLinkedInSearchUrl(extractCompanyName(x.title),jobFilter,country),country,source:'Web Search',dateFound:today(),score:0,painSignal:false,snippet:x.snippet||''});
  return leads;
}

// ⚡ PARALLEL ENRICHMENT — now 25 leads, pain signals for top 8
async function enrichLeadsParallel(leads) {
  const ENRICH_CAP = 25; // ✅ Bumped from 8 to 25
  const PAIN_CAP = 8;    // ✅ Bumped from 5 to 8
  const toEnrich = leads.slice(0, ENRICH_CAP);
  const rest = leads.slice(ENRICH_CAP);

  // Process in batches of 5 to avoid overwhelming target servers
  const BATCH_SIZE = 5;
  const enrichedLeads = [];
...
app.get('/api/health', (req, res) => res.json({ status: 'ok', version: '4.1.0', enrichCap: 25, painCap: 8, totalNiches: Object.keys(INFORMAL_CATEGORIES).length, newNiches: ['creative agency', 'pr agency', 'construction', 'construction contractor'] }));

app.listen(PORT, () => {
  console.log(`\n🚀 Lead Generator API v4.1 – port ${PORT}`);
  console.log(`⚡ Parallel scraping | 💬 WA Queue | 📊 CRM | 🎯 25-lead enrichment`);
